...
void ofApp::update(){
    ...
    for(int i = 0; i < myBirds.size(); i++){//loop through every bird
        ...
        //----------------------------------------------------------------
        float aFactor = 0.01; // aF - a multiplier for scaling the "temp acceleration" (A)
        ofPoint A;  // A
        ...
        //----------------------------------------------------------------
        float herdDependency = myBirds[i].herdDependency; // lW - find this bird's desire to explore alone
        float proximityDiscomfort = myBirds[i].proximityDiscomfort; // pD - find this bird's apathy to being in close quarters with other birds
        float flowConsideration = myBirds[i].flowConsideration; // fC - find this bird's consideration for other birds' desire to get home from work
...
void ofApp::draw(){
    ...
    //cursor glow sphere when pressed
    if(isMousePressed){
        ofPushStyle();
            ofSetColor(255, 216, 0, 25);
